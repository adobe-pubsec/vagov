...
/**
 * Transformer: VA.gov section breaks + Section Metadata.
 * Inserts <hr> before each non-first section and a Section Metadata block
 * after each section that declares a style, using payload.template.sections.
 *
 * Section selectors come from tools/importer/page-templates.json (DOM-verified
 * during page analysis). Breaks are inserted in beforeTransform (while every
 * section element still exists, before block parsers replace them) using a
 * temporary marker; metadata is inserted in afterTransform anchored to that
 * marker. Sections are processed in reverse so live-element inserts never
